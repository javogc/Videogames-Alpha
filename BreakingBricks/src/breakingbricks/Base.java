...
public class Base {
     ...
     * 
     */
    public Base(double iX, double iY , int iAncho, int iAlto, Image imaImagen) {
        this.iX = iX;
        this.iY = iY;
        this.iAncho = iAncho;
        this.iAlto = iAlto;
    ...
    public boolean intersecta (Object objBase) {
        ...
        if (objBase instanceof Base) {
            ...
            Base basTemp = (Base) objBase;
            Rectangle rctParam = new Rectangle((int) basTemp.getX(), (int) basTemp.getY(), basTemp.getAncho(),
                    basTemp.getAlto());
            
            return rctEste.intersects(rctParam);
        }
        //si no entro entonces no es Base
        return false;
    }
